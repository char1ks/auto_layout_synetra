...
class MaskFilter:
    ...
    def apply_all_filters(self, masks: List[Dict[str, Any]], image_np: np.ndarray) -> List[Dict[str, Any]]:
        ...
        # Фильтр 2: Границы изображения (по сегментации, как в hybrid)
        before = len(masks)
        ...
        # Фильтр 4: Объединение перекрывающихся масок (заглушка остается)
    ...
    # Удаляем старую реализацию обработки границ по bbox — оставляем вариант по сегментации
    ...
    def _filter_perfect_rectangles(self, masks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for mask_dict in masks:
            ...
            # Создаем идеальный прямоугольник из bbox (XYWH)
            x, y, w, h = bbox
            # Приводим к целым и ограничиваем рамками изображения
            H, W = mask.shape[:2]
            x = int(round(float(x)))
            y = int(round(float(y)))
            w = int(round(float(w)))
